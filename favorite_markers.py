...
#        return 'test'
    ...
    def add_arguments(self, pars):
        ...
        pars.add_argument("--addt", type=inkex.Boolean, default=True,help="Add template?")
        pars.add_argument("--template_name", type=str, default='untitled',help="New template name")
        pars.add_argument("--remt", type=inkex.Boolean, default=True,help="Remove template?")
        pars.add_argument("--template_rem", type=int, default=1, help="Template to remove");
    ...
    def set_marker_props(self,el,mkrname,mtype,mkrdat):
        if mkrdat is not None:
            ...
            existing = [v for v in dh.descendants2(self.svg.defs) if newname in v.get_id()];
            previousmkr = None
            for eel in existing:
                if len(eel.getchildren())>0 and isinstance(eel.getchildren()[0],(Group)):
                    trn = Transform(eel.getchildren()[0].get('transform'));
                    if abs(trn.a-mysize)<.01 and abs(trn.d-mysize)<.01:
                        previousmkr=eel;
    ...
    def effect(self):   
        ...
        import pickle, os
        fmsettings = os.path.abspath(os.path.join(get_script_path(),'favorite_markers.settings'));
        try:
        ...
        if self.options.tab=='addremove':
            ...
            fminx = os.path.abspath(os.path.join(get_script_path(),'favorite_markers.inx'));
            f=open(fminx);
            inxd = f.read();
            f.close();
