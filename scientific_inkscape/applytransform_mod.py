...
class ApplyTransform(inkex.EffectExtension):
    ...
    def recursiveFuseTransform(
        ...
        if "d" in node.attrib or isinstance(
            node, (Polygon, Polyline, Ellipse, Circle, Line, Rectangle)
        ):  # supported types
            ...
            if not (transf == NULL_TRANSFORM and irange is None and trange is None):
                ...
                if "d" in node.attrib:
                    ...
                    node, (Ellipse, Circle)
                ):  # node.tag in [inkex.addNS("ellipse", "svg"), inkex.addNS("circle", "svg")]:
