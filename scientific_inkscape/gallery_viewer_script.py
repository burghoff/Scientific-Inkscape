            ...
            with open(gloc, "rb") as f:
                ...
                for v in fileuris:
                    ...
                    if folder not in folder_dict.values():
                        ...
                        k = next(
                            key for key, value in folder_dict.items() if value == folder
                        )
                ...
                for ii, fu in enumerate(fileuris):
                    new_string = new_string.replace(
                        bytes('data-src="{0}"'.format(str(fu)), "utf-8"),
    ...
    def make_svg_display():
        ...
        with open(gloc, "w") as file:
            ...
            for wt in watcher_threads:
                svg_filenames, thumbnails, header, slidenums, islinked = (
                    wt.files[:],
                    wt.thumbnails,
                    wt.header,
                    wt.slidenums,
                ...
                if wt.run_on_fof_done:
                    for ii, svg in enumerate(svg_filenames):
                        ...
                        gallery = gallery.replace(
                            'data-src="#"',
                            'data-src="{0}" class="lazyload"'.format(tnloc),
                        )
                        ...
                        if wt.embeds is not None:
                            if wt.embeds[ii]:
                                ...
                                embed = '<br><a href="http://localhost:{0}/process?param={1}" class="open">Original</a><br>'.format(
                                    str(PORTNUMBER), embed
                                )
            ...
            if os.path.isfile(self.fof):
                ...
                if ftype == "ppt":
                    ...
                    linked = [
                        ...
                        for k in self.slidenums.keys()
                        if "file:" in k
                    ]
                    self.files += linked
                    self.slidenums = {
                        os.path.join(contents, ftype, "media", os.path.basename(k))
                        if "file:" not in k
                ...
                self.pagenums = [
                    pn if len(tnl) > 1 else None
                    for tnl in self.thumbnails
                    for pn in range(1, 1 + len(tnl))
                ]
